...
The Frontend team developed their part of the application. However, they need you to provide the data so it works properly. They gave you the following API documentation. These are the **API endpoints** that you need to implement:
...
The Database team provided you with a script to create a local database. You must implement the DAO methods to retrieve and store data in the database. The interface provides you with enough information to complete your tasks.

## Cat APIs

There are two web APIs that will provide you with sources of random cat fact and cat picture data.  Since these are essentially additional data access methods for Cat Cards, you can calls these APIs from the DAO class.  You will need both of these APIs to complete the GetRandomCatCard() method of the DAO.
